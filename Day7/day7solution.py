...
def main():
	bag_rules_input = get_input_list('day7_input.txt')
	
	bag_rules_input_dict = {}
	for r in bag_rules_input:
		key = str(r.split(' contain ')[0])
		val = str(r.split(' contain ')[1])
		print(key + ' : '+ str(get_bag_list(val)))
		bag_rules_input_dict[key] = get_bag_list(val)

	bags_containing_gold = []
	for key in bag_rules_input_dict:
...
def get_bag_list(val):
	if 'no other bags' in val: return []
